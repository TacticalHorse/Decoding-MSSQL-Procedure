--You must use DAC!
--ATTENTION: This is not a standard method of connecting to the server. Please read the connection instructions carefully.
--SSMS: File => New => DBMS Core Query => Add ADMIN: before the server name (e.g., ADMIN:MYLOCALDB)
--To obtain such access, you need the sysadmin group for accounting.
--The code is based on the article https://sqlity.net/en/1617/decrypting-encrypted-database-objects/.
--Be sure to use the correct database, otherwise the object will not be found. Replace YOURDB with your own.
...
--SSMS: Файл => Создать => Запрос ядра СУБД => Перед названием сервера дописываем ADMIN: (Например ADMIN:MYLOCALDB)
...
use[YOURDB]
--Write the name of your object. Replace YOUROBJECT with the desired name.
...
--There is a problem with byte shifting, Cyrillic characters are corrupted. 
--Bytes from @plane can be saved using HxD or similar software and worked with. 
--Bytes containing the values 0x00 and 0x04 (all either even or odd) must be moved to the beginning by 2 bytes. 
--Example code for moving bytes C# “bytes”: for (int i = 1; i < bytes.Length-1; i = i+2) bytes[i] = bytes[i+2]; where i is the bytes containing 0x00 and 0x04
--Next is the code for moving bytes that worked for me.
...
--Build a new byte array taking into account the edit
...
--print does not allow more than 4/8k characters to be output for NVARCHAR/VARCHAR types, respectively; we bypass this with a loop for output.
--print не дает вывести больше 4/8к символов для типов NVARCHAR/VARCHAR соответственно, обходим циклом на вывод
print('--------DECODED PROCEDURE--------');
